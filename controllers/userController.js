...
export async function createUsers(req, res) {
  ...
  // Check if full phone number (numbercode + number) already exists
...
export async function updateUsers(req, res) {
  ...
  if (req.body.number) updateData.number = req.body.numbercode + req.body.number;
  ...
  if (req.body.noncashballance) updateData.noncashballance = req.body.noncashballance;
  if (req.body.emailverified) updateData.emailverified = req.body.emailverified;
  if (req.body.numberverified) updateData.numberverified = req.body.numberverified;
  ...
  if (req.body.email) {
    const existingEmail = await User.findOne({ email: req.body.email, id: { $ne: userId } });
    if (existingEmail) {
      return res.status(400).json({ message: 'Email is already entered' });
    }
  }
  ...
  if (req.body.number) {
    if(req.body.number.length !== 9) {
      return res.status(400).json({ message: 'Phone number should be 9 digits', data: req.body.number });
    }
    const existingNumber = await User.findOne({ number: req.body.number, id: { $ne: userId } });
    if (existingNumber) {
      return res.status(400).json({ message: 'Phone number is already entered' });
    }
  }
  ...
  if (req.body.nic) {
    const existingNic = await User.findOne({ nic: req.body.nic, id: { $ne: userId } });
    if (existingNic) {
      return res.status(400).json({ message: 'NIC is already entered' });
    }
  }
